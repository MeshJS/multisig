...
const getWalletIdsForBotMock: jest.Mock = jest.fn();
...
beforeEach(() => {
  ...
  createCallerMock.mockReturnValue({ wallet: { getUserWallets: jest.fn() } });
  (getWalletIdsForBotMock as any).mockResolvedValue([{ walletId: "w1", walletName: "Wallet 1" }]);
});

describe("walletIds bot API", () => {
  it("returns 403 for address mismatch", async () => {
    const req = {
      method: "GET",
      headers: makeBearerAuth(),
      query: { address: "addr_test1wrong" },
  ...
  it("returns wallet ids for authorized bot", async () => {
    const req = {
      method: "GET",
      headers: makeBearerAuth(),
      query: { address: BOT_TEST_ADDRESS },
    } as unknown as NextApiRequest;
    const res = createMockResponse();
    await handler(req, res);
    expect(getWalletIdsForBotMock).toHaveBeenCalled();
    expect(res.status).toHaveBeenCalledWith(200);
    expect(res.json).toHaveBeenCalledWith([{ walletId: "w1", walletName: "Wallet 1" }]);
  });
});
