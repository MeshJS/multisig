...
export function CrowdfundInfo({ 
              ...
              if (contributorInput) {
                contributionList.push({
                  ...
                  timestamp: (() => {
                    ...
                    const blockTime = (tx as any).blockTime || (tx as any).block_time || tx.block_time;
                    ...
                    if (blockTime && !isNaN(Number(blockTime))) {
                      ...
                      console.warn(`No valid timestamp found for transaction ${(tx as any).tx_hash || 'unknown'}, using current time`);
                      return new Date();
                    }
                  })(),
                  txHash: tx.tx_hash,
  ...
  const handleCompleteCrowdfund = async () => {
    ...
    try {
      ...
      const result = await contract.completeCrowdfund({
        datum,
                ...
                {crowdfundData.recentContributions.map((contribution, index) => (
                  <div key={index} className="flex items-center justify-between p-3 bg-muted rounded-lg">
                    ...
                    <div className="text-right">
                      <div className="text-xs text-muted-foreground">
                        {dateToFormatted(contribution.timestamp)}
                      </div>
                    </div>
                  </div>
                ))}
