...
export default function CardSigners({ appWallet }: { appWallet: Wallet }) {
  ...
  return (
    <Card className="col-span-2">
      ...
      <CardTitle className="text-xl font-medium">Signers</CardTitle>
        <Key className="h-4 w-4 text-muted-foreground" />
      </CardHeader>
      <CardContent>
        <div className="mt-1 flex flex-col gap-2">
          <p className="text-sm text-muted-foreground">
            This wallet requires <b className="text-white">{appWallet.numRequiredSigners}</b> signers to sign
            a transaction.
          </p>
            ...
            {appWallet.signersAddresses.map((signer, index) => (
              <div className="flex items-center gap-4" key={signer}>
                <div className="flex gap-2 justify-center items-center">
                  ...
                  >
                    <p className="text-sm text-muted-foreground">{getFirstAndLast(signer)}</p>
                  </Button>
                </div>
              </div>
            ))}
