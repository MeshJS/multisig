...
const updateIdentity = (index: number, value: string) => {
  ...
  async function createAnchor() {
    const rawResponse = await fetch("/api/vercel-storage/put", {
      ...
      body: JSON.stringify({
        pathname: `drep/${appWallet?.address.slice(-10)}.jsonld`,
  ...
  {links.map((link, index) => (
    <div key={index} className="flex gap-2">
      ...
      />
      <Button onClick={() => removeLink(index)} size="icon" variant="destructive">X</Button>
    </div>
  ))}
  <Button onClick={addLink} variant="secondary">+ Add Link</Button>
</div>
...
<div className="grid gap-3">
  ...
  {identities.map((identity, index) => (
    <div key={index} className="flex gap-2">
      ...
      />
      <Button onClick={() => removeIdentity(index)} size="icon" variant="destructive">X</Button>
    </div>
  ))}
  <Button onClick={addIdentity} variant="secondary">+ Add Identity</Button>
</div>
        </fieldset>
