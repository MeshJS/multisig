...
import { loadAllPayableTasks, loadPayableTasks } from "./load";
import { buildPayoutSpec, recipientsHash } from "./spec";

export type TaskPayoutPreviewInput = {
  walletId: string;
  /** Omitted or empty: every payable task in the wallet. */
  taskIds?: string[];
};
 ...
 * — the named ones, or every payable one when none were named — and turned
 * into a canonical spec, and the shared preview pipeline builds, summarizes
 * and mints the draft token — with the task ids and a hash of their recipient
 * rows bound in, so `transaction_propose` / `confirmPayout` link exactly the
 * tasks whose amounts were on the card.
 *
 * Persists nothing.
 */
  ...
  try {
    ...
    const tasks =
      input.taskIds && input.taskIds.length > 0
        ? await loadPayableTasks(deps.db, wallet.walletRow.id, input.taskIds)
        : await loadAllPayableTasks(deps.db, wallet.walletRow.id);
