...
```powershell
docker compose -f docker-compose.playwright.yml build app playwright-runner
```
...
**`Missing required environment variables`** — one of the four required env vars is
missing. Check your `.env.playwright` or shell export.

**`No legacy/hierarchical/sdk wallet found`** — the bootstrap context is stale or
written by an older schema. Delete `ci-artifacts/ci-wallet-context.json` and re-run
bootstrap.
